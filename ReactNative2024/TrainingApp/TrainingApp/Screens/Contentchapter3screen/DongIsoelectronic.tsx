...
const DongIsoelectronic = () => {
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <Text style={styles.title}>Dòng Isoelectronic</Text>
        <Text style={styles.content}>
          &emsp;Các nguyên tử sẽ thu được, mất đi hoặc chia sẻ các electron hóa trị để đạt được cấu hình octet (trong hầu hết các trường hợp) hoặc cấu hình electron tương tự như cấu hình của khí hiếm. Ví dụ, tất cả các nguyên tử và ion sau đây đều có 18 electron và cấu hình electron tương tự cấu hình của khí hiếm, argon: S²⁻, Cl¹⁻, Ar, K¹⁺, và Ca²⁺. Tất cả các nguyên tử ion này đều có cấu hình electron là 1s²2s²2p⁶3s²3p⁶.
        </Text>
        <Text style={styles.content}>
          &emsp;Biết được thông tin vừa trình bày, bây giờ chúng ta có thể dự đoán kích thước tương đối của các nguyên tử và ion được trình bày. Bởi vì tất cả chúng đều có 18 electron nên chúng ta có thể xem xét số lượng proton có mặt. Ion có số proton lớn nhất, Ca, sẽ có bán kính nhỏ nhất vì nó có “lực hút” hạt nhân lớn nhất lên 18 electron. Lưu huỳnh, chỉ với 16 proton, sẽ có lực hút hạt nhân ít nhất đối với 18 electron mà nó có. Điều này giúp giải thích tại sao các nguyên tử phi kim lại nhỏ hơn các ion tương ứng của chúng. Tương tự, nó cũng giải thích tại sao các nguyên tử kim loại lại lớn hơn các ion tương ứng của chúng.
        </Text>
      </ScrollView>
    </SafeAreaView>
  );
...
export default DongIsoelectronic
