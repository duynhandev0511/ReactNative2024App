...
const Dongvi = () => {
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <Text style={styles.title}>Gọi tên các hợp chất</Text>
        <Text style={styles.content}>
          &emsp;Khả năng gọi tên các hợp chất và xác định công thức hóa học của một hợp chất xuất phát từ khả năng phân biệt giữa các hợp chất ion và cộng hóa trị. Tên của một hợp chất phụ thuộc rất nhiều vào loại liên kết giữa các nguyên tử. Ngoài việc có thể nhận biết một số loại liên kết nhất định, khi học cách gọi tên các hợp chất, tốt nhất bạn nên nhớ các quy tắc áp dụng cho loại liên kết đó. Quy tắc đặt tên cho 4 loại hợp chất thông dụng được nêu dưới đây.
        </Text>
        <Image
          source={require('../anhlythuyet/Bang5chapter5.png')}
          style={styles.image2}
          resizeMode="contain"
        />
        <Text style={styles.title}>Vấn đề</Text>
        <Text style={styles.content}>
          &emsp;Đặt tên sau đây: S²⁻, N³⁻, CaF₂, K₂S, NaOH, Na₂SO₄, SO₃, và CCl₄
        </Text>
        <Text style={styles.title}>Giải pháp</Text>
        <Text style={styles.content}>
          &emsp;Hai ion đầu tiên là các anion nguyên tử đơn và được gọi là sulfide và nitride. Hai chất tiếp theo là các hợp chất ion nhị phân, calcium fluoride và potassium sunfide. Các ion hydroxide đa nguyên tử và sunfate có trong sodium hydroxide và sodium sunfate. Cuối cùng, hai hợp chất cuối cùng được liên kết cộng hóa trị và được gọi là sulfur trioxide và carbon tetrachlorine
        </Text>     
      </ScrollView>
    </SafeAreaView>
  );
