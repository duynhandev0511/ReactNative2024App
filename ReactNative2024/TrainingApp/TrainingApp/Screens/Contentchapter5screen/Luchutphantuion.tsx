...
const Dongvi = () => {
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <Text style={styles.title}>Lực hút phân tử-ion</Text>
        <Text style={styles.content}>
          &emsp;Khi muối được cho vào nước, chúng sẽ hòa tan (một số muối nhiều hơn những muối khác) và tạo thành các ion. Sau khi muối hòa tan được hòa tan trong nước, sẽ có lực hút giữa các ion tích điện và các phân tử nước có cực. Đây được gọi là lực hút phân tử-ion. Do lực hút này, các ion ngậm nước trong dung dịch làm cho các phân tử nước tự định hướng theo một kiểu cụ thể. Phần oxygen của phân tử nước có điện tích âm vì độ âm điện cao. Phần âm này của phân tử nước sẽ bị thu hút bởi các cation trong dung dịch. Vì các nguyên tử hydrogen trong nước mang điện tích dương nhẹ nên chúng sẽ tự định hướng về phía các anion trong dung dịch. Đó là trường hợp NaCl hòa tan trong nước như hình 5.27.
        </Text>
        <Image
          source={require('../anhlythuyet/5.27.png')}
          style={styles.image2}
          resizeMode="contain"
        />
         <Text style={styles.title}>Vấn đề</Text>    
         <Text style={styles.content}>
          &emsp;Loại liên kết nào được tìm thấy giữa các phân tử và nguyên tử sau đây: F₂(g), Ne(g), KBr(aq), H₂O(l), NH₃(l), và CH₄(g)?
        </Text>  
        <Text style={styles.title}>Giải pháp</Text> 
        <Text style={styles.content}>
          &emsp;Khí fluorine là khí không phân cực và sẽ chịu lực Van der Waals. Ne là khí hiếm và cũng chịu tác dụng của lực Van der Waals. KBr(aq) là một hợp chất ion hòa tan trong nước. Lực hút giữa các ion và phân tử nước là lực phân tử-ion. Các phân tử nước và ammonia sẽ chịu cả lực lưỡng cực và liên kết hydrogen. Cuối cùng, vì khí methane là khí không phân cực nên sẽ chịu lực Van der Waals giữa các phân tử.
          </Text>  
          <Text style={styles.content}>
          &emsp;Biểu đồ này cung cấp một bản tóm tắt hữu ích về liên kết giữa các phân tử
          </Text>
          <Image
          source={require('../anhlythuyet/Bang4chapter5.png')}
          style={styles.image2}
          resizeMode="contain"
        />
      </ScrollView>
