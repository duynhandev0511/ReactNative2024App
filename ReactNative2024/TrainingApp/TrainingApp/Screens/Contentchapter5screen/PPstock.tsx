...
const Dongvi = () => {
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        <Text style={styles.title}>Phương pháp Stock</Text>
        <Text style={styles.content}>
          &emsp;Phương pháp đặt tên gốc các hợp chất giúp làm rõ tên các hợp chất có chứa các nguyên tố chuyển tiếp. Ví dụ: có hai loại clorua sắt là FeCl₂ và FeCl₃. Làm thế nào bạn có thể phân biệt cái này với cái kia? Trong FeCl2, sắt có điện tích ion là 2 ⁺, trong khi ở FeCl₃, sắt có điện tích ion là 3 ⁺. Chữ số La Mã trong ngoặc đơn biểu thị điện tích có trên cation. Vì vậy, FeCl₂ được gọi là iron(II) chloridevà FeCl₃ được gọi là iron(III) chloride. Sau khi thấy điện tích của cation, bạn có thể áp dụng các quy tắc cho phương pháp chéo.
        </Text>
        <Text style={styles.title}>Vấn đề</Text>  
        <Text style={styles.content}>
          &emsp;Công thức hóa học của tin(IV) fluoride và lead(IV) oxide?
        </Text>
        <Text style={styles.title}>Giải pháp</Text>  
        <Text style={styles.content}>
          &emsp;Sử dụng phương pháp chéo, tin(IV) fluoride will be SnF₄. Khi sử dụng phương pháp chéo cho lead(IV) oxide, ban đầu công thức sẽ giống Pb₂O₄ nhưng hãy nhớ rằng tất cả các hợp chất ion đều được viết ở tỷ lệ thấp nhất, công thức thực là PbO₂.
        </Text>        
      </ScrollView>
    </SafeAreaView>
  );
